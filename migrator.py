...
class Migrator:
    ...
    def initialize(self):
        ...
        #
        self.esxi_instance = SmartConnect(
            host=self.esxi_host,
            user="root",
            pwd=self.esxi_ssh_password,
                    ...
                    if hasattr(vm, "summary"):
                        ...
                        if vm_name == self.esxi_vm_name:
                            ...
                            for datastore in self.esxi_datastores.keys():
                                self.esxi_vm_path = self.esxi_vm_path.replace(
                                    datastore, self.esxi_datastores[datastore]
                                )

                            self.esxi_vm_path = re.sub(
                                r"\[(.+)\] ", r"\1/", self.esxi_vm_path
                            )
                            self.esxi_vm_path = os.path.dirname(
                                os.path.normpath(self.esxi_vm_path)
                            )

        if not self.esxi_vm:
            logger.error(
                f"Could not find vm on esxi host {self.esxi_host}: {self.esxi_vm}"
            )
            sys.exit(1)

        if not self.esxi_vm_path:
            logger.error("Could not determine VM path on ESXI host")
            sys.exit(1)
        ...
        #
        logger.info("Checking ssh connection to ESXI host")
        try:
        ...
        result_security_group, *_ = self.openstack_execute(
            f"security group show {self.security_group}"
        )
    ...
    def _create_ssh_client(self):
        ...
        client.connect(
            self.esxi_host,
            port=self.esxi_ssh_port,
            username="root",
            password=self.esxi_ssh_password,
        )
        return client
...
{Colors.CRITICAL}Be very sure from here on.{Colors.ENDC}
        ...
        answer = input(
            f"{Colors.CRITICAL}{Colors.BOLD}Proceed?{Colors.ENDC} {Colors.CRITICAL}[y/N]{Colors.ENDC} "
        )
        if answer.lower() != "y":
            sys.exit(2)
        ...
        stdin, stdout, stderr = ssh_client.exec_command(
            f"/bin/vim-cmd vmsvc/power.off {self.esxi_vm_id}"
        )
    ...
    def copy_images(self):
        ...
        if len(vmdk_files) > 0 and not self.arguments["forceCopy"]:
            logger.info(
                f"There are already vmdk files in {self.data_dir}, skipping (use --forceCopy to force copying)"
            )
            return
        ...
        ssh_client = self._create_ssh_client()
        scp = SCPClient(
            ssh_client.get_transport(), sanitize=lambda x: x, progress=_scp_progress
        )
    ...
    def convert_images(self):
        ...
        try:
            ...
            for vmdk_file in vmdk_files:
                ...
                os.system(
                    f"qemu-img convert -p -f vmdk -O raw {vmdk_dir}/{vmdk_file} {self.data_dir}/{vmdk_file}.raw"
                )
    ...
    def openstack_execute(self, command):
        ...
        result = subprocess.run(
            ["openstack"] + command.split(" ") + ["-f", "yaml"],
            capture_output=True,
            text=True,
            env=os_env,
        )
        ...
        for raw_file in sorted(raw_files):
            ...
            for result in result_imported:
                if result["Name"] == raw_file:
                    logger.info(
                        f"Skipping import of {raw_file}: already imported as {result['ID']}"
                    )
                    imported = True
                    result_show, *_ = self.openstack_execute(
                        f"image show {result['ID']}"
                    )
                    ...
                    if result_show["size"]:
                        ...
                        logger.error(
                            f"Failed to get image id of already imported image {raw_file}"
                        )
                        sys.exit(1)
            ...
            result_import, rso, rse = self.openstack_execute(
                "image create "
            ...
            if result_import["ID"]:
                self.images.append(
                    (
                        result_import["ID"],
                        os.stat(f"{self.data_dir}/{raw_file}").st_size,
                    )
                )
    ...
    def create_server(self):
        ...
        logger.info("Creating server with the following settings:")
        ...
        logger.info(
            f"    Boot volume with image: {boot_id}, size {size / 1024 / 1024 / 1024}GB"
        )
        opt_boot_image = (
            f"--image {boot_id} --boot-from-volume {int(size / 1024 / 1024 / 1024)}"
        )
        ...
        for image in self.images:
            image_id, size = image
            logger.info(
                f"    Additional volume with image: {image_id}, size {size / 1024 / 1024 / 1024}GB"
            )
            opt_block_devices.append(
                f"--block-device uuid={image_id}"
                f",source_type=image"
        ...
        openstack_command = (
            f"server create {opt_flavor} {opt_boot_image} {opt_security_group} "
