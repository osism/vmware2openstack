...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="VMWare to OpenStack move script",
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    parser.add_argument("-c", "--config", help="Config file to use", required=True)
    parser.add_argument("-n", "--name", help="Name of the SCS server to create during migration", required=True)
    parser.add_argument("-fc", "--forceCopy", action="store_true",
    ...
    try:
        ...
        logger.error(f"Could not read configuration {options['config']}: {error.reason}")
        sys.exit(1)
